...
export class AllBlogComponent implements OnInit {
  ...
  loadInitialData() {
    this._blogsService.getAllBlogs({ page: 1, count: 10 }).subscribe({
      next: (res) => {
  ...
  private handleResponse(res: any) {
    this.blogs = res.items?.data || [];
    this.blogsLength = res.items?.total || 0;
    this.BlogsMeta = { ...res.items };
    this.loading = false;
  }
  ...
  private loadData() {
    this.loading = true;
    this._blogsService.getAllBlogs(this.params).subscribe({
      next: (res) => {
        this.handleResponse(res);
  ...
  get isLastPage(): boolean {
    return this.params.page === this.BlogsMeta.last_page;
  }
  ...
  deleteBlog(blog: any) {
    this._confirmationService.confirm({
      ...
      rejectLabel: 'لا',
      accept: () => {
        this._blogsService.deleteBlog(blog.id).subscribe({
          next: (response) => {
            this.loadInitialData();
            this._messageService.add({
              severity: 'success',
  ...
  editBlog(blog: any) {
    this._router.navigate([`/blogs/add-blog/${blog.id}`]);
  }
  ...
  onImportFile(event) {
    if (event.target.files.length > 0) {
      ...
      this._blogsService.importBlogs(formData).subscribe({
        next: (response) => {
  ...
  onExportData(): void {
    this._blogsService.exportBlogsData().subscribe({
      next: (response: any) => {
