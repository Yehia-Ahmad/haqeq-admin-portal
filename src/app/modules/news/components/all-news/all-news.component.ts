...
export class AllNewsComponent implements OnInit {
  ...
  loadInitialData() {
    this._newsService.getAllNews({ page: 1, count: 10 }).subscribe({
      next: (res) => {
  ...
  private handleResponse(res: any) {
    this.news = res.items?.data || [];
    this.newsLength = res.items?.total || 0;
    this.NewsMeta = { ...res.items };
    this.loading = false;
  }
  ...
  private loadData() {
    this.loading = true;
    this._newsService.getAllNews(this.params).subscribe({
      next: (res) => {
        this.handleResponse(res);
  ...
  get isLastPage(): boolean {
    return this.params.page === this.NewsMeta.last_page;
  }

  deleteNews(news: any) {
    this._confirmationService.confirm({
      message: 'هل أنت متأكد أنك تريد حذف؟ ' + news.title,
      header: 'تاكيد الحذف',
  ...
  editNews(news: any) {
    this._router.navigate([`/news/add-news/${news.id}`]);
  }
  ...
  onImportFile(event) {
    if (event.target.files.length > 0) {
      ...
      this._newsService.importNews(formData).subscribe({
        next: (response) => {
  ...
  onExportData(): void {
    this._newsService.exportNewsData().subscribe({
      next: (response: any) => {
